...
for i in range(5):
    ...
    ypred.to_csv(f'./dataset/{i}.csv')
sum_f = pd.read_csv('./dataset/1.csv')
for i in range(2,5):
    try:
        da = pd.read_csv(f'./dataset/{i}.csv')
        for k in range(1, sum_f.shape[0]):
            sum_f.iloc[k, 1] += da.iloc[k, 1]
    except:
        print('没有这个文件')
sum_f.to_csv('./dataset/sum.csv')
print('共有',len(aut_list),'个模型')
aut_list = pd.DataFrame(aut_list).to_csv('./dataset/auc.csv')
